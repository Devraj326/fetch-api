...
fetch('https://api.openweathermap.org/data/2.5/weather?q=bengaluru,in&APPID=cdaf8ad0fc09f62ce13321323225efb1'
)
 ...
 .then((parsedData) => {
  ...
  document.getElementsByClassName('temperature')[0] .textContent = Math.floor (weather - 273) + '\xBA' + 'C';

  const clouds = parsedData.weather[0].main;
  document.getElementsByClassName('mausam')[0] .textContent = clouds

  const maxtemp = parsedData.main.temp_max;
  const mintemp = parsedData.main.temp_min;
  document.getElementsByClassName('tapman')[0] .textContent= Math.floor(maxtemp - 273) + 
  '\xBA' +
  ' ' +
  'C' +
  ' ' + 
